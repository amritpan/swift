...
static SILFunction *getOrCreateUnappliedKeypathEnumCase(
    SILGenModule &SGM, EnumElementDecl *method, SubstitutionMap subs,
    GenericEnvironment *genericEnv, ResilienceExpansion expansion,
    ArrayRef<IndexTypePair> args, CanType baseType, CanType methodType) {
  //  lookupMethodViaProtocol(method, subs);
  ...
  // Mangle the name of the thunk to see if we already created it.
  auto name = Mangle::ASTMangler(SGM.getASTContext(), method)
                  .mangleKeyPathUnappliedEnumCaseThunkHelper(
                      method, genericSig, baseType, subs, expansion);
  auto loc = RegularLocation::getAutoGeneratedLocation();
  auto thunk =
      getOrCreateKeypathThunk(SGM, name, signature, genericEnv, expansion, loc);
  ...
  ManagedValue resultSubst;
  {
    RValue resultRValue = subSGF.emitSimpleKeyPathEnumCase(
        loc, baseSubstValue, baseType, method, methodType,
        std::move(preparedArgs), subs, SGFContext());
...
KeyPathPatternComponent SILGenModule::emitKeyPathComponentForDecl(
  ...
  if (auto *storage = dyn_cast<EnumElementDecl>(decl)) {
    EnumElementDecl *externalDecl = nullptr;
    SubstitutionMap externalSubs;
    CanType componentTy;
    auto enumEltTy = decl->getInterfaceType()->castTo<AnyFunctionType>();
    if (isApplied) {
      // If method is applied, set component type to method result type.
      auto enumResultTy =
          enumEltTy->getResult()->castTo<AnyFunctionType>()->getResult();
      if (auto genMethodTy = enumResultTy->getAs<GenericFunctionType>())
        enumResultTy = genMethodTy->substGenericArgs(subs);
      componentTy = enumResultTy->mapTypeOutOfContext()->getCanonicalType();
    } else {
      // Otherwise, component type is method type without Self.
      if (auto genMethodTy = enumEltTy->getAs<GenericFunctionType>())
        enumEltTy = genMethodTy->substGenericArgs(subs);
      auto methodInterfaceTy = cast<AnyFunctionType>(
          enumEltTy->mapTypeOutOfContext()->getCanonicalType());
    ...
    SILDeclRef representative(storage, SILDeclRef::Kind::EnumElement,
    ...
    //          *this, storage, subs, needsGenericContext ? genericEnv :
    //          nullptr, expansion, argTypes, baseTy, componentTy);
    //    } else {
    func = getOrCreateUnappliedKeypathEnumCase(
    ...
    //                                              componentTy);
  } else if (auto *storage = dyn_cast<AbstractFunctionDecl>(decl)) {
...
RValue RValueEmitter::visitKeyPathExpr(KeyPathExpr *E, SGFContext C) {
  ...
  for (size_t i = 0; i < components.size(); i++) {
    ...
    case KeyPathExpr::Component::Kind::Subscript: {
      ...
      if (auto enumElt = dyn_cast<EnumElementDecl>(decl);
          enumElt && i + 1 < components.size() &&
          components[i + 1].getKind() == KeyPathExpr::Component::Kind::Apply) {
        argComponent = components[i + 1];
        isApplied = true;
      }
