...
static SILFunction *
getOrCreateKeyPathMethod(SILGenModule &SGM, AbstractFunctionDecl *method,
                         SubstitutionMap subs, GenericEnvironment *genericEnv,
                         ResilienceExpansion expansion,
                         ArrayRef<IndexTypePair> args, CanType baseType,
                         CanType methodType, bool isApplied) {
  ...
  auto signature = [&]() {
    ...
    //    SILResultInfo result(loweredMethodTy, ResultConvention::Unowned);
...
KeyPathPatternComponent SILGenModule::emitKeyPathComponentForDecl(
  ...
  if (auto *storage = dyn_cast<AbstractFunctionDecl>(decl)) {
    ...
    if (auto decl = dyn_cast<FuncDecl>(storage)) {
      ...
      //          expansion, argTypes, baseTy, componentTy, isApplied);
