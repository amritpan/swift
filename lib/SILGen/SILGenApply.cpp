...
RValue SILGenFunction::emitRValueForKeyPathMethod(
    ...
    SubstitutionMap substitutions, SGFContext C) {
  ...
  Callee callee = emitSpecializedAccessorFunctionRef(
      *this, loc, SILDeclRef(method), substitutions, selfArgSource, false, true,
      false);

  CallEmission emission(*this, std::move(callee), std::move(writebackScope));

  // Self
  emission.addSelfParam(loc, std::move(selfArgSource),
                        methodArgs.getParams()[0]);
